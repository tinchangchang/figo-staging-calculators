  ...
  function calculate(values) {
    ...
    if (missing.length) {
      ...
      return {
        status: 'incomplete',
        anatomicStage: '',
        displayStage: 'Not determined',
        reason: `Complete ${missing.length} remaining staging ${missing.length === 1 ? 'field' : 'fields'} to avoid understaging.`,
        rationale: missing.map(label => `Select: ${label}.`),
        badges: [`${requiredFields.length - missing.length} of ${requiredFields.length} complete`],
        missing
      };
    }

    return addMolecularDesignation(calculateAnatomicStage(values), values);
  }
